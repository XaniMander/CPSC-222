...
private:
    ...
    *  radian arguments, instead of degrees.
    */
...
public:
	...
	double sumCalc(vector<vector<double>>& sumTable, vector<double>& tempRow, double currCell, int i);
	double avgCalc(vector<vector<double>>& sumTable, pair<int,int>& ul, pair<int,int>& lr);
